            ...
            getBoundSession(slackThread)
                .then(function (doc) {
                    ...
                    if (doc.length === 0 || !doc[0].active) {
                        return reject('No active session on thread. Start using: start session [taskid]');
                    }
                    ...
                    session.attendance.forEach(function (a) {
                        if (a.account._id == account._id) {
                            reject('You\'ve already joined the session ' + mixins.getDiffMins(moment(a.joined), moment()).toFixed(2) + 'm ago');
                            return;
                        }
                    });
